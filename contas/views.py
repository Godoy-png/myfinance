...
def definir_contas(request):
    if request.method == "GET":
        ...
        titulo = request.POST.get('titulo').upper()
...
def ver_contas(request):
    ...
    contas_proximas_vencimento = contas.filter(
        dia_pagamento__lte=DIA_ATUAL + 5).filter(dia_pagamento__gt=DIA_ATUAL).exclude(id__in=contas_pagas)

    restantes = contas.exclude(id__in=contas_vencidas).exclude(
        id__in=contas_proximas_vencimento).exclude(id__in=contas_pagas)
