...
def novo_valor(request):
    ...
    messages.add_message(request, constants.SUCCESS, 'Movimentação Adicionada com Sucesso!!')
...
def view_extrato(request):
    ...
    contas_get = request.GET.get('conta')
    categoria_get = request.GET.get('categoria')

    valores = Valores.objects.filter(data__month=datetime.now().month)
    
    if contas_get:
        valores = valores.filter(conta__id = contas_get)
